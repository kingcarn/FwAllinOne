// =============UserScript=============
// @name        Trakt 追剧日历
// @description 内置 API Key 版：只需填写用户名即可使用。
// @author      MakkaPakka 
// =============UserScript=============

var WidgetMetadata = {
    id: "trakt_personal_key_tv",
    title: "Trakt 追剧日历 免key版",
    author: "𝙈𝙖𝙠𝙠𝙖𝙋𝙖𝙠𝙠𝙖",
    description: "内置 API Key 版：只需填写用户名即可使用。显示追剧日历、待看、收藏及历史记录。",
    version: "1.1.10", // 升级版本号
    requiredVersion: "0.0.1",
    site: "https://t.me/MakkaPakkaOvO",

    globalParams: [
        { name: "traktUser", title: "Trakt 用户名 (全局)", type: "input", value: "" }
    ],
    ...
    modules: [
        {
            ...
            cacheDuration: 300,
            params: [
                // 💡 修复核心1：给 TV 用户准备的专属输入口
                { name: "traktUser", title: "Trakt 用户名 (TV用户填这,填一次即可)", type: "input", value: "" },
                {
                    name: "section",
                    title: "浏览区域",
                    type: "enumeration",
...
async function loadTraktProfile(params = {}) {
    // 💡 修复核心2：TV端终极防丢缓存逻辑
    let traktUser = (params.traktUser || "").trim();

    if (traktUser) {
        // 只要能抓到一次有效输入，立马锁进本地存储
        try { Widget.storage.set("trakt_username_cache", traktUser); } catch(e) {}
    } else {
        // 如果被 TV 端 App 吞了，就从本地存储里强行捞出来
        try { traktUser = Widget.storage.get("trakt_username_cache") || ""; } catch(e) {}
    }

    if (!traktUser) {
        return [{ id: "err", type: "text", title: "请在过滤菜单中填写 Trakt 用户名" }];
    }

    const { section, updateSort = "future_first", type = "all", page = 1 } = params;
